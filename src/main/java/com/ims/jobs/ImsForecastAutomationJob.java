...
public class ImsForecastAutomationJob implements Job {
	...
	void runForecast(StringBuilder url, RestTemplate restTemplate, TicketStatistics statistics) {
		try {
			if (StatusType.COMPLETED.getDescription().equalsIgnoreCase(statistics.getAutomationStatus())
					&& StatusType.OPEN.getDescription().equalsIgnoreCase(statistics.getForecastStatus())) {
